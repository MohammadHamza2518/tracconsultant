...
export default function TbToBalanceSheetPage() {
  ...
  const handleDownloadPdfSummary = () => {
    ...
    try {
      ...
      doc.text('Schedule III Statutory Financial Statements (MCA & Companies Act 2013)', margin, 18);
      ...
      // Badge
      doc.setFillColor(isBsBalanced ? 5 : 225, isBsBalanced ? 150 : 29, isBsBalanced ? 105 : 72);
      doc.roundedRect(pageWidth - margin - 52, 14, 52, 6, 1, 1, 'F');
      doc.setTextColor(255, 255, 255);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(7);
      doc.text(isBsBalanced ? 'BALANCE SHEET: BALANCED' : 'BALANCE SHEET: MISMATCH', pageWidth - margin - 26, 18.2, { align: 'center' });
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(9);
      doc.text(`ENTITY: ${companyName.toUpperCase()} | FINANCIAL YEAR: ${financialYear}`, margin + 3, currentY + 4.8);
      currentY += 10;
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8.5);
      doc.text('PART II: STATEMENT OF PROFIT AND LOSS', margin + 3, currentY + 4.5);
      currentY += 7.5;

      const pnlRows = [
        ['I. Revenue from Operations (Gross Sales)', `Rs. ${totalRevenue.toLocaleString('en-IN')}`],
        ['II. Total Operating, Admin & Finance Expenses', `Rs. ${totalExpenses.toLocaleString('en-IN')}`],
        ['III. Net Profit / (Loss) Transferred to Reserves', `Rs. ${netProfit.toLocaleString('en-IN')}`]
      ];

      pnlRows.forEach((r, idx) => {
        const isNet = idx === pnlRows.length - 1;
        doc.setFillColor(isNet ? 240 : (idx % 2 === 0 ? 255 : 248), isNet ? 253 : (idx % 2 === 0 ? 255 : 250), isNet ? 244 : (idx % 2 === 0 ? 255 : 252));
        ...
        doc.setFontSize(8);
        doc.setTextColor(isNet ? 5 : 71, isNet ? 150 : 85, isNet ? 105 : 105);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8.5);
      doc.text('PART I: BALANCE SHEET - EQUITY AND LIABILITIES', margin + 3, currentY + 4.5);
      currentY += 7.5;

      const eqRows = [
        ["(a) Shareholders' Funds (Equity Capital + Net Profit)", `Rs. ${totalEquity.toLocaleString('en-IN')}`],
        ['(b) Non-Current Liabilities (Term Loans & Borrowings)', `Rs. ${nonCurrentLiabilities.toLocaleString('en-IN')}`],
        ['(c) Current Liabilities (Trade Payables, GST/TDS & Provisions)', `Rs. ${currentLiabilities.toLocaleString('en-IN')}`],
        ['TOTAL EQUITY & LIABILITIES', `Rs. ${totalLiabilities.toLocaleString('en-IN')}`]
      ];

      eqRows.forEach((r, idx) => {
        const isTot = idx === eqRows.length - 1;
        doc.setFillColor(isTot ? 238 : (idx % 2 === 0 ? 255 : 248), isTot ? 242 : (idx % 2 === 0 ? 255 : 250), isTot ? 255 : (idx % 2 === 0 ? 255 : 252));
        doc.rect(margin, currentY, contentWidth, 6, 'F');
        doc.setDrawColor(226, 232, 240);
        doc.rect(margin, currentY, contentWidth, 6, 'S');
        ...
        doc.setFontSize(8);
        doc.setTextColor(isTot ? 67 : 71, isTot ? 56 : 85, isTot ? 202 : 105);
        doc.text(r[0], margin + 3, currentY + 4.2);
        doc.text(r[1], pageWidth - margin - 3, currentY + 4.2, { align: 'right' });
        currentY += 6;
      });
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8.5);
      doc.text('PART I: BALANCE SHEET - ASSETS', margin + 3, currentY + 4.5);
      currentY += 7.5;

      const assetRows = [
        ['(a) Non-Current Assets (Property, Plant & Equipment)', `Rs. ${fixedAssets.toLocaleString('en-IN')}`],
        ['(b) Current Assets (Trade Receivables, Stock, Cash & Bank)', `Rs. ${currentAssets.toLocaleString('en-IN')}`],
        ['TOTAL ASSETS', `Rs. ${totalAssets.toLocaleString('en-IN')}`]
      ];

      assetRows.forEach((r, idx) => {
        const isTot = idx === assetRows.length - 1;
        doc.setFillColor(isTot ? 236 : (idx % 2 === 0 ? 255 : 248), isTot ? 253 : (idx % 2 === 0 ? 255 : 250), isTot ? 245 : (idx % 2 === 0 ? 255 : 252));
        doc.rect(margin, currentY, contentWidth, 6, 'F');
        doc.setDrawColor(226, 232, 240);
        doc.rect(margin, currentY, contentWidth, 6, 'S');
        ...
        doc.setFont('helvetica', isTot ? 'bold' : 'normal');
        doc.setFontSize(8);
        doc.setTextColor(isTot ? 5 : 71, isTot ? 150 : 85, isTot ? 105 : 105);
        doc.text(r[0], margin + 3, currentY + 4.2);
        doc.text(r[1], pageWidth - margin - 3, currentY + 4.2, { align: 'right' });
        currentY += 6;
      });
      ...
      // Balance status box
      doc.setFillColor(isBsBalanced ? 236 : 254, isBsBalanced ? 253 : 242, isBsBalanced ? 245 : 242);
      doc.roundedRect(margin, currentY, contentWidth, 12, 1.5, 1.5, 'F');
      doc.setDrawColor(isBsBalanced ? 16 : 239, isBsBalanced ? 185 : 68, isBsBalanced ? 129 : 68);
      doc.roundedRect(margin, currentY, contentWidth, 12, 1.5, 1.5, 'S');

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8);
      doc.setTextColor(isBsBalanced ? 4 : 185, isBsBalanced ? 120 : 28, isBsBalanced ? 87 : 28);
      doc.text(`BALANCE SHEET STATUS: ${isBsBalanced ? 'PERFECTLY BALANCED (Assets = Liabilities)' : `MISMATCH DIFFERENCE: Rs. ${Math.abs(totalAssets - totalLiabilities).toLocaleString('en-IN')}`}`, margin + 3, currentY + 5);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(7);
      doc.setTextColor(100, 116, 139);
      doc.text('Prepared in accordance with Division I / Division II of Schedule III to the Companies Act, 2013.', margin + 3, currentY + 9);
      ...
      doc.text('TracConsultant Financial Statements Suite • https://tracconsultant.com', margin, currentY);
      doc.text(`Generated: ${new Date().toLocaleString('en-IN')}`, pageWidth - margin, currentY, { align: 'right' });

      doc.save(`Schedule_III_Report_${companyName.replace(/[^a-zA-Z0-9]/g, '_')}_${financialYear.replace(/\s+/g, '')}.pdf`);
    } catch (err) {
      console.error('Failed to generate Schedule III PDF:', err);
    }
  };
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm border border-slate-200 space-y-3">
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex flex-col sm:flex-row gap-2">
                ...
                <button
                  ...
                  className="py-2.5 px-3 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-xl shadow-md transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                >
